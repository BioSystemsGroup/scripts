...
##
## Time-stamp: <2018-04-03 17:55:15 gepr>
###
...
for (snap in argv) {
  ...
  sxyv <- as.data.frame(as.cimg(as.vector(as.matrix(s[,2:ncol(s)]))/NUM.STATES,x=ncol(s)-1,y=length(s[,1])))
  ## append the color column
  sxyvc <- cbind(sxyv,rep(3,length(sxyv)))
  ...
  png(imFile,width=ncol(s),height=nrow(s))
  par(mar=rep(0,4)) ## no margins!
  plot(as.cimg(i),interpolate=F,colourscale=cscale,rescale=F,axes=F)
  dev.off()
