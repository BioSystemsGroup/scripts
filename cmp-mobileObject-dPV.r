...
#
# Time-stamp: <2017-05-03 15:41:20 gepr>
#
...
if (length(argv) < 2) {
    print("Usage: cmp-mobileObject-dPV.r <experiment names>")
    print("  e.g. cmp-mobileObject-dPV.r exp000x exp001x ...")
    print("  Files like dPV.mobileObject/exp000x_mobileObject_dPV∈[0,5]-totals.csv should exist.")
    print("Note that columns and band sizes must match across all files.")
    quit()
}


PATH <- "dPV.mobileObject"
exps <- argv
fnbase <- "_mobileObject_dPV∈["
fnsuff <- "]-totals.csv"
...
for (rndx in seq(0,max(bounds[,2])-band,band)) {
  ...
  for (column in columns[2:length(columns)]) {
    ...
    outputFile <- paste("graphics/mobileObject-", paste(exps,collapse="_"),title.suff,".png",sep="")
