...
if (length(argv) < 2) {
    print("Usage: cmp-hsol-dPV.r <experiment names>")
    print("  e.g. cmp-hsol-dPV.r exp000x exp001x ...")
    print("  Files like dPV.hsol/exp000x_hsolute_dPV∈[0,5]-totals.csv should exist.")
...
for (rndx in seq(0,max(bounds[,2])-band,band)) {
  ...
  for (column in columns[2:length(columns)]) {
    ...
    outputFile <- paste("graphics/hsol-", paste(exps,collapse="_"),title.suff,".png",sep="")
